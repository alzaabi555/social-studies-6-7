...
// غيرنا Map إلى MapPinned لأنه أكثر أماناً وموجود في كل النسخ
import { GraduationCap, Library, Globe, Star, FileQuestion, User, School, Phone, ChevronLeft, MapPinned, BookOpen } from 'lucide-react';

// ==========================================
// 1. تعريف المكون الفرعي (GradeCard) أولاً
// (وضعه في الأعلى يمنع مشاكل الشاشة البيضاء)
// ==========================================
...
const GradeCard: React.FC<GradeCardProps> = ({ title, desc, icon, color, bgColor, borderColor, onClick }) => (
  <button 
    onClick={onClick}
    className={`group relative overflow-hidden rounded-3xl p-6 text-right border border-slate-700 bg-slate-800/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-xl ${borderColor}`}
  >
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onSelectGrade, onOpenQuestionBank }) => {
  return (
    // استخدمنا bg-slate-950 وهو لون قياسي آمن جداً
    <div className="min-h-screen w-full bg-slate-950 relative overflow-hidden font-tajawal text-right flex flex-col" dir="rtl">
      ...
      <div className="relative z-10 container mx-auto px-6 py-12 flex flex-col flex-grow items-center justify-center">
        ...
        <div className="text-center mb-16 space-y-6 max-w-3xl">
          <div className="relative inline-block group">
            ...
            <div className="relative w-28 h-28 bg-slate-800 border border-slate-700 rounded-3xl flex items-center justify-center shadow-2xl">
        ...
        <button 
          ...
          className="group relative w-full max-w-xl bg-slate-800 border border-slate-700 p-1 rounded-2xl overflow-hidden hover:border-indigo-500 transition-all duration-300 hover:shadow-2xl"
        >
          <div className="relative flex items-center justify-between p-5 rounded-xl bg-slate-900/50 backdrop-blur-sm">
      ...
      {/* الفوتر */}
      <footer className="relative z-10 w-full border-t border-slate-800 bg-slate-900/90 backdrop-blur-md py-6">
