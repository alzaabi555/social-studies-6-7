...
// قمنا بتغيير Map إلى MapIcon لتجنب التعارض البرمجي
import { GraduationCap, Library, Globe, Star, FileQuestion, User, School, Phone, ChevronLeft, Map as MapIcon, BookOpen } from 'lucide-react';
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onSelectGrade, onOpenQuestionBank }) => {
  return (
    // استخدمنا كود لون مباشر bg-[#0f172a] بدلاً من slate-950 لضمان ظهور الخلفية الداكنة
    <div className="min-h-screen bg-[#0f172a] relative overflow-hidden font-tajawal text-right flex flex-col" dir="rtl">
      
      {/* 1. الخلفية (مبسطة لتجنب مشاكل العرض) */}
      ...
      <div className="relative z-10 container mx-auto px-6 py-12 flex flex-col flex-grow items-center justify-center">
        ...
        <button 
          ...
          className="group relative w-full max-w-2xl bg-[#1e293b] border border-slate-700 p-1 rounded-2xl overflow-hidden hover:border-indigo-500 transition-all duration-300 hover:shadow-2xl"
        >
          <div className="relative flex items-center justify-between p-5 rounded-xl bg-[#1e293b]/80 backdrop-blur-sm">
      ...
      {/* 5. الفوتر */}
      <footer className="relative z-10 w-full border-t border-slate-800 bg-[#0f172a]/90 backdrop-blur-md py-6">
...
// المكون الفرعي (آمن وبدون استخدام any)
...
const GradeCard: React.FC<GradeCardProps> = ({ title, desc, icon, color, bgColor, borderColor, onClick }) => (
  <button 
    onClick={onClick}
    className={`group relative overflow-hidden rounded-3xl p-6 text-right border border-slate-700 bg-[#1e293b]/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-xl ${borderColor}`}
  >
